...
*   **FR5.3:** Приложение должно отображать новые заказы, их детали (состав, столик/тип заказа) и позволять менять их статус (новый, принят, готовится, готов, выполнен).
...
*   **Основной поток:**
    ...
    5. Клиент подтверждает заказ и выбирает способ оплаты (онлайн/официанту), столик переходит в статус "Занят".
    6. Система принимает заказ, присваивает ему статус "Новый".
    7. Заказ появляется в приложении для сотрудников и на кухонном дисплее.
    8. После проведения оплаты пользователю начисляются баллы.
...
*   **Основной поток:**
    ...
    5. При получении заказа кухней статус меняется на "Готовится"
    6. Когда заказ готов, кухня меняет статус на "Готов".
    7. Официант видит уведомление и доставляет заказ клиенту.
    8. После доставки официант отмечает заказ как "Выполнен".
